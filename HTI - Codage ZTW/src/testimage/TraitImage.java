...
public abstract class TraitImage {
   ...
   static BufferedImage toBufferedImage(Image image) {
        ...
        if( image instanceof BufferedImage ){
                ...
                /* On s'assure que l'image est complftement chargee */
...
*/ 
   ...
   public static double[][] Autocorelation(double[][] donnee,int w,int h){
    ...
    double[][] R=new double[r.length*r[0].length-1][r.length*r[0].length-1];

    for(int i=r.length-1;i>=0;i--)
	for(int j=r[0].length-1;j>=0;j--)
	    for(int k=r.length-1;k>=0;k--)
		for(int l=r[0].length-1;l>=0;l--)
		    {
			if (!( (i==0) && (j==0) ))
			    {
				if (!( (k==0) && (l==0) ))
				{
				    //System.out.println("R["+Integer.toString((r.length-1-i)*r[0].length+(r[0].length-1-j))+"]["+Integer.toString((r.length-1-k)*r[0].length+(r[0].length-1-l))+"]=r["+Math.abs(i-k)+"]["+Math.abs(j-l)+"];");
				    R[(r.length-1-i)*(r[0].length)+(r[0].length-1-j)][(r.length-1-k)*(r[0].length)+(r[0].length-1-l)]=r[Math.abs(i-k)][Math.abs(j-l)];
				 }
			    }
		    }
...
public static double[] getV(double[][] r){
    double[] V=new double[r.length*r[0].length-1];
	...
	for(int j=0;j<r[0].length;j++)
	    {
		    ...
		    {
			V[i*r[0].length+j]=r[r.length-1-i][r[0].length-1-j];
		    }
	    }
    return V;
}
